    ...
    <PageLayout>
      ...
      <section className="relative overflow-hidden py-28 lg:py-36">
        ...
        <div className="relative z-10 max-w-4xl mx-auto px-6 text-center">
          <motion.h1 initial={{ opacity: 0, y: 30 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.5 }} className="font-display text-4xl md:text-6xl font-bold text-primary-foreground mb-6">Our Story</motion.h1>
          <motion.p initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.5, delay: 0.15 }} className="font-body text-lg text-primary-foreground/70 max-w-2xl mx-auto">Olive Foods (Pvt) Ltd — Sri Lanka's trusted import, bonded warehousing & FMCG distribution partner for over three decades.</motion.p>
        </div>
      </section>
      ...
      <section className="py-20 lg:py-28 bg-background/90 backdrop-blur-sm">
        <div className="max-w-4xl mx-auto px-6">
          <motion.div initial={{ opacity: 0, y: 30 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ duration: 0.5 }} className="space-y-6">
            <h2 className="font-display text-3xl md:text-4xl font-bold text-foreground">Who We Are</h2>
      ...
      <section className="py-20 lg:py-28 bg-background/80 backdrop-blur-sm">
        <div className="max-w-6xl mx-auto px-6">
          <motion.h2 initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ duration: 0.5 }} className="font-display text-3xl md:text-4xl font-bold text-foreground text-center mb-16">Core Business Activities</motion.h2>
          <motion.div variants={container} initial="hidden" whileInView="show" viewport={{ once: true }} className="grid sm:grid-cols-2 lg:grid-cols-3 gap-5">
            {businessActivities.slice(0, 3).map((a) => (
              <motion.div key={a.title} variants={item} className="p-6 rounded-lg border border-border bg-card/90 backdrop-blur-sm hover:border-forest-mid/30 hover:shadow-lg transition-all duration-300">
                <div className="w-12 h-12 rounded-lg bg-forest-deep/10 flex items-center justify-center mb-4"><a.icon className="w-6 h-6 text-forest-mid" /></div>
      ...
      <section className="py-20 lg:py-28 bg-background/80 backdrop-blur-sm">
        <div className="max-w-5xl mx-auto px-6">
          <motion.h2 initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ duration: 0.5 }} className="font-display text-3xl md:text-4xl font-bold text-foreground text-center mb-16">Our Competitive Advantage</motion.h2>
          <motion.div variants={container} initial="hidden" whileInView="show" viewport={{ once: true }} className="grid sm:grid-cols-2 lg:grid-cols-4 gap-5">
            {advantages.map((v) => (
              <motion.div key={v.title} variants={item} className="text-center space-y-4 p-5 rounded-lg border border-border bg-card/90 backdrop-blur-sm hover:border-forest-mid/20 hover:shadow-lg transition-all duration-300">
                <div className="w-12 h-12 rounded-lg bg-forest-deep/5 flex items-center justify-center mx-auto"><v.icon className="w-7 h-7 text-forest-mid" /></div>
      ...
      <section className="py-20 lg:py-28 bg-background/90 backdrop-blur-sm">
        <div className="max-w-5xl mx-auto px-6">
          <motion.h2 initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ duration: 0.5 }} className="font-display text-3xl md:text-4xl font-bold text-foreground text-center mb-6">Who We Serve</motion.h2>
          <motion.p initial={{ opacity: 0, y: 15 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} className="font-body text-muted-foreground text-center mb-16 max-w-2xl mx-auto">We partner with businesses across Sri Lanka's food and hospitality ecosystem</motion.p>
          <motion.div variants={container} initial="hidden" whileInView="show" viewport={{ once: true }} className="grid grid-cols-2 sm:grid-cols-3 gap-5">
            {clientSegments.map((s) => (
              <motion.div key={s.label} variants={item} className="flex flex-col items-center gap-4 p-6 rounded-lg border border-border bg-card/90 backdrop-blur-sm hover:border-forest-mid/20 hover:shadow-lg transition-all duration-300">
                <div className="w-12 h-12 rounded-xl bg-forest-deep/10 flex items-center justify-center"><s.icon className="w-6 h-6 text-forest-mid" /></div>
      ...
      <section className="py-20 lg:py-28 bg-background/90 backdrop-blur-sm">
        <div className="max-w-6xl mx-auto px-6">
          <motion.h2 initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} className="font-display text-3xl md:text-4xl font-bold text-foreground text-center mb-12">Our Location</motion.h2>
          <div className="grid lg:grid-cols-[1fr_1.5fr] gap-10 items-start">
            <motion.div initial={{ opacity: 0, x: -30 }} whileInView={{ opacity: 1, x: 0 }} viewport={{ once: true }} className="space-y-5">
              <div><h4 className="font-display text-lg font-semibold text-foreground mb-1">Address</h4><p className="text-muted-foreground font-body text-sm leading-relaxed">Olive Foods (Pvt) Ltd<br />Colombo, Sri Lanka</p></div>
              <div><h4 className="font-display text-lg font-semibold text-foreground mb-1">Phone</h4><p className="text-muted-foreground font-body text-sm">+94 11 234 5678</p></div>
              <div><h4 className="font-display text-lg font-semibold text-foreground mb-1">Email</h4><p className="text-muted-foreground font-body text-sm">info@olivefoods.lk</p></div>
              <div><h4 className="font-display text-lg font-semibold text-foreground mb-1">Hours</h4><p className="text-muted-foreground font-body text-sm">Mon – Fri: 8:30 AM – 5:30 PM</p></div>
            </motion.div>
            <motion.div initial={{ opacity: 0, x: 30 }} whileInView={{ opacity: 1, x: 0 }} viewport={{ once: true }} className="rounded-lg overflow-hidden border border-border shadow-lg">
              <iframe src="https://www.google.com/maps/embed?pb=!1m18!1m12!1m3!1d126743.58585953498!2d79.7861!3d6.9271!2m3!1f0!2f0!3f0!3m2!1i1024!2i768!4f13.1!3m3!1m2!1s0x3ae253d10f7a7003%3A0x320b2e4d32d3838d!2sColombo%2C%20Sri%20Lanka!5e0!3m2!1sen!2sus!4v1700000000000!5m2!1sen!2sus" width="100%" height="350" style={{ border: 0, overflow: "hidden" }} allowFullScreen loading="lazy" referrerPolicy="no-referrer-when-downgrade" title="Olive Foods Location" className="w-full h-[350px] block" scrolling="no" />
            </motion.div>
          </div>
        </div>
      </section>
      ...
      <section className="py-20 lg:py-28 relative overflow-hidden">
        ...
        <div className="max-w-3xl mx-auto px-6 text-center space-y-8 relative z-10">
          <motion.h2 initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} className="font-display text-3xl md:text-4xl font-bold text-primary-foreground">Ready to Partner With Us?</motion.h2>
          <motion.p initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ delay: 0.1 }} className="font-body text-primary-foreground/60 text-lg">Let's discuss how Olive Foods can support your business with reliable import and distribution solutions.</motion.p>
          <motion.div initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ delay: 0.2 }} className="flex flex-wrap gap-4 justify-center">
            <Link to="/products"><Button className="bg-accent text-accent-foreground hover:bg-accent/90 font-body font-semibold rounded-xl px-8 h-12 text-base">Explore Products</Button></Link>
            <Link to="/brands"><Button variant="outline" className="border-primary-foreground/20 text-primary-foreground hover:bg-primary-foreground/10 font-body rounded-xl px-8 h-12 text-base">View Brands</Button></Link>
          </motion.div>
        </div>
      </section>
    </PageLayout>
