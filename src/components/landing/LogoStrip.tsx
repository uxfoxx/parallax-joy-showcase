          ...
          <motion.div className="flex animate-marquee" style={{ x: leftShift }}>
            {leftLogos.map((logo, i) => (
              <div key={`l-${i}`} className="flex-shrink-0 mx-10 flex items-center justify-center h-12 px-8">
                <span className="font-display text-xl font-bold text-primary-foreground/20 hover:text-primary-foreground/50 transition-all duration-500 whitespace-nowrap hover:drop-shadow-[0_0_12px_hsl(75_38%_45%_/_0.3)]">
                  {logo}
                </span>
              </div>
            ))}
